...
//When Methods of same name but different parameters or TYPE(int,double,float,etc) of parameters in a SAME class is used as Method overloading. You will call the method according to the type of parameter or number of parameters according to your needs.
...
class Calculator {
    ...
    public int add(int n1, int n2, int n3) { // here we used the same method(add) and changes the number of parameters
        return n1 + n2 + n3;
    }

    public double add(int n1, double n2) { // here we used the same method(add) and changed the type of parameters
        return n1 + n2;
    }
}
